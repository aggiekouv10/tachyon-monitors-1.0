...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.CHAMPSSPORTSCA);
const arc = new webhook.Webhook('https://discord.com/api/webhooks/976344414433726544/O1Fkg5IOBZrv-kGfPtwHRESlzKmWCLsUIlOjAU5hX-z2FmS7JQEo1uxoyoRG63hLz-Av');
const helper = require('../helper');
const { v4 } = require('uuid');
const DATABASE_TABLE = 'champssportsca';
const SITENAME = 'CHAMPSSPORTSCA'
...
async function monitor(sku) {
    ...
    fetch(`https://www-champssports-ca.translate.goog/zgw/product-core/v1/pdp/sku/${sku}?_x_tr_sl=en&_x_tr_tl=el&_x_tr_hl=en&_x_tr_pto=wapp`, {
        'headers': {
        ...
        if (body.inventory.inventoryAvailable == true) {
            ...
            let url = `https://www.champssports.ca/product/~/${sku}.html#Tachyon`
            let sizes = []
...
async function postRestockWebhook(url, title, sku, price, image, sizeright, sizeleft, count) {
    var zephyraio = `https://api.zephyraio.com/api/quicktask?url=https://www.champssports.ca/product/~/${sku}.html`
    var prism = 'https://prismaio.com/dashboard?url=https://www.champssports.ca/product/~/' + sku + '.html'
    var Polaris = 'http://localhost:9099/footsites?store=champssportsca&sku=' + sku + '&platform=desktop'
    var kylin = `https://dashboard.kylinbot.io/quick-task/kylin-bot/create?input=https://www.champssports.ca/product/~/${sku}.html&sku=${sku}`
    var cyber = 'https://cybersole.io/dashboard/tasks?quicktask=champssportsca:' + sku
    var EVE = 'http://remote.eve-backend.net/api/v2/quick_task?link=https://www.champssports.ca/&sku=' + sku
    var Ganesh = 'https://ganeshbot.com/api/quicktask?STORE=CHAMPSSPORTSCA%20US&PRODUCT=' + sku + '&SIZE=ANY'
    var Whatbot = 'https://whatbot.club/redirect-qt?qt=whatbot://https://www.champssports.ca/product/~/A4159800' + sku + '.html'
    var PD = 'https://api.destroyerbots.io/quicktask?url=https://www.champssports.ca/product/~/A4159800' + sku + '.html'
    var MekAIO = `https://dashboard.mekrobotics.com/quicktask?link=https://www.champssports.ca/product/~/${sku}.html&sku=${sku}`
    var wrath = 'https://whatbot.club/redirect-qt?qt=whatbot://https://www.champssports.ca/product/~/' + sku + '.html'
    var hayha = `http://localhost:7447/qt?url=https://www.champssports.ca/product/~/${sku}.html`
    var venetia = `http://localhost:4444/quicktask?storetype=Footsites&input=https://www.champssports.ca/product/~/${sku}.html`
    var kage = `http://localhost:1007/quickTasks?site=champssportsca&sku=${sku}`
    var webhookMessage = new webhook.MessageBuilder()
        .setName("Tachyon Monitors")
        .setColor("#6cb3e3")
        .setTitle(title)
        .setURL(url)
        .setAuthor('https://www.champssports.ca', '', 'https://www.champssports.ca')
        ...
        .setFooter("ChampsSports CA | v2.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
