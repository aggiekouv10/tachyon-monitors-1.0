...
let CAPTCHA = [];
let PRODUCTS = {}
let useragent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.93 Safari/537.36'
let cookie = ''
...
async function monitor(sku, proxy = getProxy()) {
    let url = `https://www.walmart.com/ip/tachyon/${sku}`;
    let productCache = PRODUCTS[sku]
    if (!productCache)
        return;
        async function pxgen() {
        let response2 = await got('https://slapio.com/px/gen', {
                method: "POST",
                headers: {
            ...
            let body2 = await response2.body
            body2 = await JSON.parse(body2)
            useragent = await body2.result.useragent
            cookie = await '_px3=' + body2.result._px3 + '; _pxvid='+ body2.result.vid + 'com.wm.reflector=reflectorid:0000000000000000000000;'
        }
        pxgen()
