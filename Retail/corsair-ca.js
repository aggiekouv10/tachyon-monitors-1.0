...
const DATABASE_TABLE = 'corsairca';
let PRODUCTS = {}
let stats;
// ^^ THESE

const archook = new webhook.Webhook('https://discord.com/api/webhooks/917279970785656873/NLRneokHRWol0tQQfUK4Z58k1R7eYmP3kyZ6qA53CmSRCW83cFA5tdpGGOokqgo3c4Bc')
...
async function monitor(sku) {
    ...
    try {
    await page.goto(`https://www.corsair.com/us/ca/p/json/${sku}`, { waitUntil: 'networkidle' });
    let body = await page.textContent('pre')
    body = JSON.parse(body.trim());
    let status = productCache.status
    stats.success++
    if (body.stock.stockLevel > 0) {
        let url = `https://www.corsair.com/ca/en/Categories/Products/tachyon/p/${sku}`
        let stock = body.stock.stockLevel
        let title = body.name
        let price = body.price.formattedValue
        let image = 'https://www.corsair.com' + body.images[0].url
        if (status !== "In-Stock") {
...
async function postRestockWebhook(url, title, sku, price, image, stock) {
    ...
    var webhookMessage = new webhook.MessageBuilder()
        ...
        .setTitle(title)
        .setURL(url)
        .setAuthor('https://www.corsair.com/ca/en', '', 'https://www.corsair.com/ca/en')
        .addField("**Stock**", stock, true)
        .addField("**Price**", price, true)
        .addField("**SKU**", sku, true)
        .setThumbnail(image)
        .setAvatar("https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829")
        .setFooter("Corsair CA | v1.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
