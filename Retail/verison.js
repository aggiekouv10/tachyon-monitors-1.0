...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.VERIZON);
const CHANNEL = discordBot.channels.VERIZON;
const helper = require('../helper');
const DATABASE_TABLE = 'verizon';
...
async function startMonitoring() {
    ...
    console.log('[VERISON] Monitoring all SKUs!')
}
async function monitor(sku) {
    ...
    fetch(`https://scache-ws.vzw.com/digital/inv/prd/${realsku}.json&abcz=${v4()}`, {
        'headers': {
        ...
        if (body.entitledItems) {
            if (body.bjsItemsInventory[0].availInventory === true) {
                ...
                if (status !== "In-Stock") {
                    ...
                    console.log(`[VERISON] In Stock! ${sku}`)
...
discordBot.getClient.on('message', async function (msg) {
    ...
    if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
        if (msg.channel.id === CHANNEL) {
            ...
            embed.setTitle(`VERISON Monitor`);
