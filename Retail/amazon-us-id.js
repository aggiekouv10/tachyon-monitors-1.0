...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.AMAZONID);
const spacehook = new webhook.Webhook('https://discord.com/api/webhooks/891719470240903178/y5mA-ZPv_dKY0OHz2PzEuZ9ruZUwQR5VEdb80-yzkjcJeBLYl-dflg3IytAEMNRmsAx9');
const eaaco = new webhook.Webhook('https://discord.com/api/webhooks/896931228085792768/-MmJOb2P3YKIp-Z-6bYZxYQXXe3eo_dOrECHBsraoeBQxcCY8sCKKzWVU5b6FjjioBFO')
...
const CHANNEL = discordBot.channels.AMAZONID;
const helper = require('../helper');
const DATABASE_TABLE = 'amazonusid';
...
async function monitor(sku) {
    ...
    fetch('https://www.amazon.com/gp/overlay/display.html', {
        ...
        const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000)
    fetch(`https://www.amazon.com/gp/aws/cart/add.html?OfferListingId.1=${sku}&Quantity.1=1&tag=mms321-20&abcz=${v4()}`, {
            "headers": {
                "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                "accept-language": "en-US,en;q=0.9",
                "cache-control": "max-age=0",
...
signal: controller.signal
        ...
        if (root.querySelector('.item-row')) {
            ...
            let url = `https://www.amazon.com/dp/${realsku}/ref=as_li_tl?ie=UTF8&camp=1789&creative=9325&creativeASIN=${realsku}&linkCode=as2&tag=tachyon03-20`;
            if (status !== "In-Stock") {
...
async function postRestockWebhook(url, title, sku, price, image, realsku) {
    let ATC = `https://www.amazon.com/gp/aws/cart/add.html?OfferListingId.1=${sku}&Quantity.1=1&tag=mms321-20`
    ...
    var webhookMessage = new webhook.MessageBuilder()
        ...
        // .setTime()
        .setFooter("Amazon US ID | v3.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
    await discordWebhook.send(webhookMessage);
    await spacehook.send(webhookMessage);
    await eaaco.send(webhookMessage);
