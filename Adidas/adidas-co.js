...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.ADIDASCO);
const helper = require('../helper');
const { v4 } = require('uuid');
const urlbase = 'https://www.adidas.co'
const currency = ' (COP)'
...
async function startMonitoring() {
  monitor(true)
  console.log('[ADIDAS-CO] Monitoring!')
}
...
async function monitor(justStarted) {
  ...
  const controller = new AbortController();
    ...
    fetch(`${urlbase}/api/plp/content-engine?query=calzado&sort=newest-to-oldest&abcz=${v4()}`, {
    'headers': {
