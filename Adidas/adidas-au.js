...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.ADIDASAU);
const helper = require('../helper');
const { v4 } = require('uuid');
const urlbase = 'https://www.adidas.com.au'
const currency = ' (ARS)'
...
async function startMonitoring() {
  monitor(true)
  console.log('[ADIDAS-AU] Monitoring!')
}
...
async function monitor(justStarted) {
  ...
  const controller = new AbortController();
    ...
    fetch(`${urlbase}/api/plp/content-engine?query=shoes&sort=newest-to-oldest&abcz=${v4()}`, {
    'headers': {
