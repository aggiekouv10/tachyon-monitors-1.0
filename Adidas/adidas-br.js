...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.ADIDASBR);
const helper = require('../helper');
const { v4 } = require('uuid');
const urlbase = 'https://www.adidas.com.br'
const currency = ' (BRL)'
...
async function startMonitoring() {
  monitor(true)
  console.log('[ADIDAS-BR] Monitoring!')
}
...
async function monitor(justStarted) {
  ...
  const controller = new AbortController();
    ...
    fetch(`${urlbase}/api/plp/content-engine?query=calcados&sort=newest-to-oldest&abcz=${v4()}`, {
    'headers': {
...
signal: controller.signal
    ...
    let products = root.raw.itemList.items
    for (let product of products) {
      let sku = product.productId
