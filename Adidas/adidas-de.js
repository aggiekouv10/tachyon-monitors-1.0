...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.ADIDASDE);
const helper = require('../helper');
const { v4 } = require('uuid');
const urlbase = 'https://www.adidas.de'
const currency = ' (EUR)'
...
async function startMonitoring() {
  monitor(true)
  console.log('[ADIDAS-DE] Monitoring!')
}
...
async function monitor(justStarted) {
  ...
  const controller = new AbortController();
    ...
    fetch(`${urlbase}/api/plp/content-engine?query=schuhe&sort=newest-to-oldest&abcz=${v4()}`, {
    'headers': {
