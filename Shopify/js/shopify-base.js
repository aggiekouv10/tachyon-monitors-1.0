...
const NOTWORKINGPROXIES = [
    "http://pcxkrtvq-dest:jvsjwc2a1kyk@93.190.245.72:9098",
    "http://pcxkrtvq-dest:jvsjwc2a1kyk@93.190.245.120:9146",
    "http://pcxkrtvq-dest:jvsjwc2a1kyk@45.155.71.0:8589",

]

const SHOPIFY_PASSWORD_HOOK = new webhook.Webhook(discordBot.webhooks.SHOPIFYPASSWORD);
const SHOPIFY_CHECKPOINT_HOOK = new webhook.Webhook(discordBot.webhooks.SHOPIFYCHECKPOINT);
const SHOPIFY_UNFILTERED_HOOK = new webhook.Webhook(discordBot.webhooks.UNFILTERED);
class ShopifyMonitor {
    ...
    async monitorProducts(page = 1, limit = 250) {
        ...
        const URL = this.WEBSITE + `/products.json?page=${page}&limit=${limit}&order=${v4()}`;  //Or you can use ?collection or ?a or ?q
            ...
            for (let product of body.products) {
                ...
                for (let variant of product.variants) {
                    if (variant.available && !variants.includes(variant.id)) {
                        ...
                        sizes += `឵឵[${variant.title}](${this.WEBSITE}/cart/${variant.id}:1) | [QT](http://localhost:6776/?url=${this.WEBSITE}/cart/${variant.id}:1) (1+)\n`
                        price = variant.price;
                    }
                }
                ...
                if (webhookType) {
                    ...
                    postRestockWebhook(this.WEBHOOK, this.WEBSITE, this.WEBSITE + "/products/" + product.handle, product.title, webhookType, sizes, oldSizes, price, product.images[0] ? product.images[0].src : "https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829", getTime(true))
                    }
...
async function postRestockWebhook(discordWebhook, website, url, title, type, sizes, oldSizes, price, image, time) {
    ...
    var webhookMessage = new webhook.MessageBuilder()
        ...
        .setTitle(title)
        .setURL(url)
        .setAuthor(website, '', url)
        .addField("**Type**", type, true)
        .addField("**Price**", price);
        sizes = sizes.split("\n")
        for(let i = 0; i < sizes.length - 1; i += 2) {
            webhookMessage.addField("sizes", sizes[i] + '\n' + sizes[i+1], true)
        }
        // .addField("**Old Variants**", oldSizes, true)
        webhookMessage.setThumbnail(image)
        .setAvatar("https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829")
        //   .setTime()
        .setFooter("Shopify | v0.9 | " + time, 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
    try {
        await discordWebhook.send(webhookMessage);
    } catch (err) {
        if (err.toString().includes("must be under"))
            console.log("[SHOPIFY] WEBHOOK OVERFLOW!")
        else
            console.log(err)
    }
}

async function postWebhook(discordWebhook, website, url, title, time) {
    // console.log(arguments)
    var webhookMessage = new webhook.MessageBuilder()
        .setName("Tachyon Monitors")
        .setColor("#6cb3e3")
