...
func getProductEmbed(restockType string, product Product, site *Site) string {
	...
	jsonString := fmt.Sprintf(`
		  {
			...
			"fields": [
			  ...
			  },
			  {
				"name": "Price",
				"value": "%s"
			  }`, strings.ReplaceAll(product.Title, `"`, `\"`), site.URL+"/products/"+product.Handle, restockType, price)
	...
	t := time.Now()
	stamp := fmt.Sprintf("%d %s %d:%d:%d.%d", t.Day(), t.Month(), t.Hour(), t.Minute(), t.Second(), (int64)(t.Nanosecond()/1000000))
	image := "https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829"
	if len(product.Images) > 0 && product.Images[0].SRC != "" {
		image = product.Images[0].SRC
	}
...
func distributeEmbedsB(webhook string, embeds []string) {
	embedArray := embeds[0]
...
func sendEmbeds(webhookURL string, embeds []string) {
	webhooks := []string{}
	embedArray := embeds[0]
	...
	for _, webhook := range webhooks {
		...
		req, err := http.NewRequest("POST", webhookURL, bytes.NewBuffer([]byte(webhook)))
...
func distributeEmbeds(webhookURL string, embeds []string) {
	embedArray := embeds[0]
	for i := 1; i < len(embeds); i++ {
		embedArray += `,` + embeds[i]
	}
	jsonString := `{"url":` + webhookURL + `, "embeds": [` + embedArray + `]}`
	_ = jsonString
}
