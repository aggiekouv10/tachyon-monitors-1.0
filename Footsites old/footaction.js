...
const { JSDOM } = jsdom;
const discordWebhook = new webhook.Webhook(discordBot.webhooks.FOOTACTION);
const helper = require('../helper');
const HTTPSProxyAgent = require('https-proxy-agent');

const DATABASE_TABLE = 'footaction';
let totalData = 0;
...
async function startMonitoring() {
  ...
  console.log("[FOOTACTION] Started monitoring all SKUs!")
}

// async function monitor(sku) {
//   let url = `https://www.footaction.com/product/tachyon/${sku}.html`;
...
async function monitor(sku) {
  let url = `https://www.footaction.com/product/tachyon/${sku}.html`;
  let pdpURL = `https://www.footaction.com/api/products/pdp/${sku}`;
  let query = await database.query(`SELECT * from ${DATABASE_TABLE} where sku='${sku}'`);
  if (query.rows.length === 0)
    return;
...
signal: controller.signal
    ...
    try {
      ...
      if(body.includes('561 Proxy Unreachable')) {
        console.log('[FOOTACTION] Proxy Fucking Unreachable - ' + sku + ' - ' + proxy);
        monitor(sku);
        return;
      }
      if(body.toLowerCase().includes('many requests') || response.status === 429) {
        console.log('[FOOTACTION] 429, SKU: ' + sku);
        setTimeout(function () {
          monitor(sku);
        }, helper.getRandomNumber(300, 700));
        return;
      }
      console.log("********************FOOTACTION-ERROR********************")
      console.log("SKU: " + sku);
    ...
    if (!body.variantAttributes) {
      if (body.url && body.url.includes('geo.captcha')) {
        console.log("[FOOTACTION] DATADOME ANTI-BOT: " + proxy);
        monitor(sku);
        return;
      }
      if (query.rows.length > 0) {
      ...
      // console.log("[FOOTACTION] OOS/Invalid SKU: " + sku);
      // console.log(body);
      return;
    }
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var phantom = 'https://api.ghostaio.com/quicktask/send?site=FootAction&input=https://www.footaction.com/product/~/' + sku + '.html'
  var tks = 'https://thekickstationapi.com/quick-task.php?link=https://www.footaction.com/product/~/' + sku + '.html&autostart=true'
  var prism = 'https://prismaio.com/dashboard?url=https://www.footaction.com/product/~/' + sku + '.html'
  var Polaris = 'http://localhost:9099/footsites?store=footaction&sku=' + sku + '&platform=desktop'
  var cyber = 'https://cybersole.io/dashboard/tasks?quicktask=Footaction:' + sku
  var EVE = 'http://remote.eve-backend.net/api/v2/quick_task?link=https://www.footaction.com/&sku=' + sku
  var Ganesh = 'https://ganeshbot.com/api/quicktask?STORE=FOOTACTION&PRODUCT=' + sku + '&SIZE=ANY'
  var Whatbot = 'https://whatbot.club/redirect-qt?qt=whatbot://https://www.footaction.com/product/~/' + sku + '.html'
  var PD = 'https://api.destroyerbots.io/quicktask?url=https://www.footaction.com/product/~/' + sku + '.html'
  var wrath = 'http://localhost:32441/qt?input=https://www.footaction.com/product/~/' + sku + '.html'
  var webhookMessage = new webhook.MessageBuilder()
    .setName("Tachyon Monitors")
    .setColor("#6cb3e3")
    .setTitle(title)
    .setURL(url)
    .setAuthor('https://www.footaction.com', '', 'https://www.footaction.com')
    ...
    .setTime()
    .setFooter("Footaction | v1.0", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
