...
async function startMonitoring() {
  ...
  console.log("[CHAMPSPORTS] Started monitoring all SKUs!")
}

async function monitor(sku) {
  let url = `https://www.champssports.com/product/tachyon/${sku}.html`;
  let pdpURL = `https://www.champssports.com/api/products/pdp/${sku}`;
...
signal: controller.signal
    ...
    try {
      ...
      if(body.includes('561 Proxy Unreachable')) {
        console.log('[CHAMPSPORTS] Proxy Fucking Unreachable - ' + sku + ' - ' + proxy);
        monitor(sku);
        return;
      }
      if(body.toLowerCase().includes('many requests') || response.status === 429) {
        console.log('[CHAMPSPORTS] 429, SKU: ' + sku);
        setTimeout(function () {
          monitor(sku);
    ...
    if (!body.variantAttributes) {
      if (body.url && body.url.includes('geo.captcha')) {
        console.log("[CHAMPSPORTS] DATADOME ANTI-BOT: " + proxy);
        monitor(sku);
        return;
      }
      if (query.rows.length > 0) {
      ...
      // console.log("[CHAMPSPORTS] OOS/Invalid SKU: " + sku);
      // console.log(body);
      return;
    }
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var phantom = 'https://api.ghostaio.com/quicktask/send?site=ChampsSports&input=https://www.champssports.com/product/spacenotify/' + sku + '.html'
  var tks = 'https://thekickstationapi.com/quick-task.php?link=https://www.champssports.com/product/spacenotify/' + sku + '.html&autostart=true'
  var prism = 'https://prismaio.com/dashboard?url=https://www.champssports.com/product/spacenotify/' + sku + '.html'
  var Polaris = 'http://localhost:9099/footsites?store=champs&sku=' + sku + '&platform=desktop'
  var cyber = 'https://cybersole.io/dashboard/tasks?quicktask=Champs%20Sports:' + sku
  var EVE = 'http://remote.eve-backend.net/api/v2/quick_task?link=https://www.champssports.com/&sku=' + sku
  var Ganesh = 'https://ganeshbot.com/api/quicktask?STORE=CHAMPSSPORTS&PRODUCT=' + sku + '&SIZE=ANY'
  var Whatbot = 'https://whatbot.club/redirect-qt?qt=whatbot://https://www.champssports.com/product/spacenotify/' + sku + '.html'
  var PD = 'https://api.destroyerbots.io/quicktask?url=https://www.champssports.com/product/spacenotify/' + sku + '.html'
  var wrath = 'http://localhost:32441/qt?input=https://www.champssports.com/product/spacenotify/' + sku + '.html'
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTime()
    .setFooter("ChampsSports | v1.0 |", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
...
discordBot.getClient.on('message', async function (msg) {
  ...
  if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
    if (msg.channel.id === discordBot.channels.CHAMPSSPORTS) {
