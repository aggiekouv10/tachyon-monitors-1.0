...
const { JSDOM } = jsdom;
const discordWebhook = new webhook.Webhook(discordBot.webhooks.FOOTLOCKERUK)
const helper = require('../helper');

const DATABASE_TABLE = 'footlockeruk';
...
async function monitor(sku) {
  let url = `https://www.footlocker.co.uk/en/p/tachyon-440?v=${sku}`;
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTitle(title)
    .setURL(url)
    .setAuthor('https://www.footlocker.co.uk/', '', 'https://www.footlocker.co.uk/')
    ...
    .setTime()
    .setFooter("Footlocker UK | v1.0 |", 'https://media.discordapp.net/attachments/801630138831470593/805010577092247552/uNgDTBW_13.png?width=611&height=611')
...
discordBot.getClient.on('message', async function (msg) {
  ...
  if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
    if (msg.channel.id === discordBot.channels.FOOTLOCKERUK) {
      let query = await database.query(`SELECT * from ${DATABASE_TABLE}`);
      const embed = new Discord.MessageEmbed();
      embed.setTitle("Footlocker UK Monitor");
