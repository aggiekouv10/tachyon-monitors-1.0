...
const DATABASE_TABLE = 'footlockerca';//Add to database
...
async function monitor(sku) {
  let url = `https://www.footlocker.ca/product/tachyon/${sku}.html`;
  let pdpURL = `https://www.footlocker.ca/api/products/pdp/${sku}`;
  ...
  const controller = new AbortController();
    ...
    if (inStock) {
      ...
      let image = `https://images.footlocker.com/is/image/EBFL2/${sku}_a1?wid=630&hei=630&fmt=png-alpha`;
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTime()
    .setFooter("Footlocker CA | v1.0 |", 'https://media.discordapp.net/attachments/801630138831470593/805010577092247552/uNgDTBW_13.png?width=611&height=611')
...
discordBot.getClient.on('message', async function (msg) {
  ...
  if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
    if (msg.channel.id === discordBot.channels.FOOTLOCKERCA) {
