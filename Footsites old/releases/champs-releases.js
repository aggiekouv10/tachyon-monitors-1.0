...
async function startMonitoring() {
  monitor()
  console.log('[Champs-Releases] Monitoring!')
}

async function monitor() {
  const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000)
    fetch("https://www.champssports.com/release-dates", {
    'headers': {
    ...
    for (let product of products) {
      ...
      if (!PRODUCTS.includes(name)) {
        ...
        if(product.querySelector('.ReleaseProduct-Link')) {
          let url = 'https://www.champssports.com' + product.querySelector('.ReleaseProduct-Link').attributes.href.replace(' ','')
          ...
          let url = `https://www.champssports.com/release-dates`
          let sku = 'NA'
