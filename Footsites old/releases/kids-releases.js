...
async function startMonitoring() {
  monitor()
  console.log('[Kids-Releases] Monitoring!')
}

async function monitor() {
  const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000)
    fetch("https://www.kidsfootlocker.com/release-dates", {
    'headers': {
    ...
    for (let product of products) {
      ...
      if (!PRODUCTS.includes(name)) {
        ...
        if(product.querySelector('.ReleaseProduct-Link')) {
          let url = 'https://www.footaction.com' + product.querySelector('.ReleaseProduct-Link').attributes.href.replace(' ','')
