...
async function startMonitoring() {
  monitor()
  console.log('[Eastbay-Releases] Monitoring!')
}

async function monitor() {
  const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000)
    fetch("https://www.eastbay.com/release-dates", {
    'headers': {
    ...
    for (let product of products) {
      ...
      if (!PRODUCTS.includes(name)) {
        ...
        if(product.querySelector('.ReleaseProduct-Link')) {
          let url = 'https://www.eastbay.com' + product.querySelector('.ReleaseProduct-Link').attributes.href.replace(' ','')
          ...
          let url = `https://www.eastbay.com/release-dates`
          let sku = 'NA'
          let image = `https://images.footlocker.com/is/image/EBFL2/317352650680`
