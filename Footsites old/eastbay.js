...
const { JSDOM } = jsdom;
const discordWebhook = new webhook.Webhook(discordBot.webhooks.EASTBAY);
const helper = require('../helper');
const HTTPSProxyAgent = require('https-proxy-agent');

const DATABASE_TABLE = 'eastbay';
let totalData = 0;
...
async function startMonitoring() {
  ...
  console.log("[EASTBAY] Started monitoring all SKUs!")
}


async function monitor(sku) {
  let url = `https://www.eastbay.com/product/tachyon/${sku}.html`;
  let pdpURL = `https://www.eastbay.com/api/products/pdp/${sku}`;
...
signal: controller.signal
    ...
    try {
      ...
      if(body.includes('561 Proxy Unreachable')) {
        console.log('[EASTBAY] Proxy Fucking Unreachable - ' + sku + ' - ' + proxy);
        monitor(sku);
        return;
      }
      if(body.toLowerCase().includes('many requests') || response.status === 429) {
        console.log('[EASTBAY] 429, SKU: ' + sku);
        setTimeout(function () {
          monitor(sku);
        }, helper.getRandomNumber(300, 700));
        return;
      }
      console.log("********************EASTBAY-ERROR********************")
      console.log("SKU: " + sku);
    ...
    if (!body.variantAttributes) {
      if (body.url && body.url.includes('geo.captcha')) {
        console.log("[EASTBAY] DATADOME ANTI-BOT: " + proxy);
        monitor(sku);
        return;
      }
      if (query.rows.length > 0) {
      ...
      // console.log("[EASTBAY] OOS/Invalid SKU: " + sku);
      // console.log(body);
      return;
    }
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  ...
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTitle(title)
    .setURL(url)
    .setAuthor('https://www.eastbay.com', '', 'https://www.eastbay.com')
    ...
    .setTime()
    .setFooter("EastBay | v1.0 |", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
...
discordBot.getClient.on('message', async function (msg) {
  ...
  if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
    if (msg.channel.id === discordBot.channels.EASTBAY) {
      let query = await database.query(`SELECT * from ${DATABASE_TABLE}`);
      const embed = new Discord.MessageEmbed();
      embed.setTitle("EastBay Monitor");
