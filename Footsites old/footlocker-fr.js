...
const { JSDOM } = jsdom;
const discordWebhook = new webhook.Webhook(discordBot.webhooks.FOOTLOCKERFR)
const helper = require('../helper');

const DATABASE_TABLE = 'footlockerfr';
...
async function monitor(sku) {
  let url = `https://www.footlocker.fr/fr/p/tachyon-87822?v=${sku}`;
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTitle(title)
    .setURL(url)
    .setAuthor('https://www.footlocker.fr/', '', 'https://www.footlocker.fr/')
    ...
    .setTime()
    .setFooter("Footlocker FR | v1.0 |", 'https://media.discordapp.net/attachments/801630138831470593/805010577092247552/uNgDTBW_13.png?width=611&height=611')
...
discordBot.getClient.on('message', async function (msg) {
  ...
  if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
    if (msg.channel.id === discordBot.channels.FOOTLOCKERFR) {
