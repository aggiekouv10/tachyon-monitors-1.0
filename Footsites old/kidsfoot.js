...
async function startMonitoring() {
  ...
  console.log("[KIDSFOOTLOCKER] Started monitoring all SKUs!")
}

// async function monitor(sku) {
//   let url = `https://www.kidsfootlocker.com/product/tachyon/${sku}.html`;
...
async function monitor(sku) {
  let url = `https://www.kidsfootlocker.com/product/tachyon/${sku}.html`;
  let pdpURL = `https://www.kidsfootlocker.com/api/products/pdp/${sku}`;
  let query = await database.query(`SELECT * from ${DATABASE_TABLE} where sku='${sku}'`);
  if (query.rows.length === 0)
    return;
...
signal: controller.signal
    ...
    try {
      ...
      if(body.includes('561 Proxy Unreachable')) {
        console.log('[KIDSFOOTLOCKER] Proxy Fucking Unreachable - ' + sku + ' - ' + proxy);
        monitor(sku);
        return;
      }
    ...
    if (!body.variantAttributes) {
      if (body.url && body.url.includes('geo.captcha')) {
        console.log("[KIDSFOOTLOCKER] DATADOME ANTI-BOT: " + proxy);
        monitor(sku);
        return;
      }
      if (query.rows.length > 0) {
      ...
      // console.log("[KIDSFOOTLOCKER] OOS/Invalid SKU: " + sku);
      // console.log(body);
      return;
    }
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var phantom = 'https://api.ghostaio.com/quicktask/send?site=KidsFootLocker&input=https://www.kidsfootlocker.com/product/spacenotify/' + sku + '.html'
  var tks = 'https://thekickstationapi.com/quick-task.php?link=https://www.kidsfootlocker.com/product/spacenotify/' + sku + '.html&autostart=true'
  var prism = 'https://prismaio.com/dashboard?url=https://www.kidsfootlocker.com/product/spacenotify/' + sku + '.html'
  var Polaris = 'http://localhost:9099/footsites?store=footlocker_kids&sku=' + sku + '&platform=desktop'
  var cyber = 'https://cybersole.io/dashboard/tasks?quicktask=Kids%20Footlocker:' + sku
  var EVE = 'http://remote.eve-backend.net/api/v2/quick_task?link=https://www.kidsfootlocker.com/&sku=' + sku
  var Ganesh = 'https://ganeshbot.com/api/quicktask?STORE=KIDS%20FTL&PRODUCT=' + sku + '&SIZE=ANY'
  var Whatbot = 'https://whatbot.club/redirect-qt?qt=whatbot://https://www.kidsfootlocker.com/product/spacenotify/' + sku + '.html'
  var PD = 'https://api.destroyerbots.io/quicktask?url=https://www.kidsfootlocker.com/product/spacenotify/' + sku + '.html'
  var wrath = 'http://localhost:32441/qt?input=https://www.kidsfootlocker.com/product/spacenotify/' + sku + '.html'
  var webhookMessage = new webhook.MessageBuilder()
    .setName("Tachyon Monitors")
    .setColor("#6cb3e3")
    .setTitle(title)
    .setURL(url)
    .setAuthor('https://www.kidsfootlocker.com', '', 'https://www.kdisfootlocker.com')
    .addField("**Stock**", "In Stock", true)
    .addField("**Price**", price, true)
    .addField("**Sizes**", sizes)
    .addField("**Sku**", sku, true)
    .addField("**Links**", '[Phantom](' + phantom + ') | ' + '[TKS](' + tks + ') | ' + '[Prism](' + prism + ') | ' + '[Polaris](' + Polaris + ') | ' + '[Cyber](' + cyber + ') |')
    .addField('', '[EVE](' + EVE + ') | ' + '[Ganesh](' + Ganesh + ') | ' + '[Whatbot](' + Whatbot + ') | ' + '[PD](' + PD + ') | ' + '[Wrath](' + wrath + ') |')
    .setThumbnail(image)
    .setAvatar("https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829")
    .setTime()
    .setFooter("Kids Footlocker | v1.0 |", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
...
discordBot.getClient.on('message', async function (msg) {
  ...
  if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
    if (msg.channel.id === discordBot.channels.KIDSFOOTLOCKER) {
      let query = await database.query(`SELECT * from ${DATABASE_TABLE}`);
      const embed = new Discord.MessageEmbed();
      embed.setTitle("Kids Footlocker Monitor");
