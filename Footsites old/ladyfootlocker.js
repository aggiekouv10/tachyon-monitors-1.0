...
const { JSDOM } = jsdom;
const discordWebhook = new webhook.Webhook(discordBot.webhooks.LADYFOOTLOCKER);
const helper = require('../helper');
const HTTPSProxyAgent = require('https-proxy-agent');

const DATABASE_TABLE = 'ladyfootlocker';
let totalData = 0;
...
async function startMonitoring() {
  ...
  console.log("[LADYFOOTLOCKER] Started monitoring all SKUs!")
}


async function monitor(sku) {
  let url = `https://www.ladyfootlocker.com/product/tachyon/${sku}.html`;
  let pdpURL = `https://www.ladyfootlocker.com/api/products/pdp/${sku}`;
...
signal: controller.signal
    ...
    try {
      ...
      if(body.includes('561 Proxy Unreachable')) {
        console.log('[LADYFOOTLOCKER] Proxy Fucking Unreachable - ' + sku + ' - ' + proxy);
        setTimeout(function () {
          monitor(sku);
      ...
      if(body.toLowerCase().includes('many requests') || response.status === 429) {
        console.log('[LADYFOOTLOCKER] 429, SKU: ' + sku);
        monitor(sku);
        return;
      }
      console.log("********************LADYFOOTLOCKER-ERROR********************")
      console.log("SKU: " + sku);
    ...
    if (!body.variantAttributes) {
      if (body.url && body.url.includes('geo.captcha')) {
        console.log("[LADYFOOTLOCKER] DATADOME ANTI-BOT: " + proxy);
        monitor(sku);
        return;
      }
      if (query.rows.length > 0) {
      ...
      // console.log("[LADYFOOTLOCKER] OOS/Invalid SKU: " + sku);
      // console.log(body);
      return;
    }
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTitle(title)
    .setURL(url)
    .setAuthor('https://www.ladyfootlocker.com', '', 'https://www.ladyfootlocker.com')
    ...
    .setTime()
    .setFooter("Lady Footlocker | v1.0", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
...
discordBot.getClient.on('message', async function (msg) {
  ...
  if (msg.content.startsWith(discordBot.commandPrefix + 'monitorList')) {
    if (msg.channel.id === discordBot.channels.LADYFOOTLOCKER) {
