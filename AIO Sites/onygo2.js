...
const snkrgroup = new webhook.Webhook('https://discord.com/api/webhooks/970089302556029008/2QeCl5e3Qee4xtM8C5x_EJgCkiA7hKVcLPKSdBsuWjpgx6GaML4E4UOAp-nbM5Fivx0D')
const helper = require('../helper');
const HTTPSProxyAgent = require('https-proxy-agent');
const { url } = require('inspector');
const randomUseragent = require('random-useragent');
const DATABASE_TABLE = 'onygo';
const SITENAME = 'ONYGO'
...
async function startMonitoring() {
  ...
  console.log("[ONYGO] Started monitoring all SKUs!")
}

async function monitor(sku) {
  let url = `https://www.onygo.com/p/tachyon-${sku}.html`
  ...
  fetch(`https://www-onygo-com.translate.goog/p/jordan-wmsn_air_jordan_1_mid_se_-lt_orewood_brn%2F%2F-${sku}.html?abcz=${v4()}&dwvar_1_size=1&format=ajax&_x_tr_sl=el&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp`, {
    "headers": {
    ...
    for (let size of sizesparse) {
      if (size.isOrderable === true) {
        sizes += `[${size.displayValue}](https://www.onygo.com${size.url})` + '\n'
        varients += `${size.variantId.trim()}\n`
        sizeList.push(size.displayValue);
        if (!oldSizeList.includes(size.displayValue)) {
          inStock = true;
        }
      }
    }
    ...
    if (inStock) {
      postRestockWebhook(url, title, sku, sizes, varients, price, image);
    }
    if (query.rows.length > 0) {
      setTimeout(function () {
        monitor(sku);
...
async function postRestockWebhook(url, title, sku, sizes, varients, price, image) {
  let flare = `[Flare](http://127.0.0.1:5559/quicktask?product=https://www.onygo.com/p/tachyon-${sku}.html)`
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .addField("**Varients**", varients, true)
    .addField(" ", " ", true)
    .addField("**QT**", flare, true)
    .setThumbnail(image)
    .setAvatar("https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829")
    .setTime()
    .setFooter("Onygo | v2.1 |", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
