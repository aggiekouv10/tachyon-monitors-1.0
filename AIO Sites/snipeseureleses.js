...
async function startMonitoring() {
  monitor()
  console.log('[Snipes-Releases] Monitoring!')
}
...
async function monitor() {
  ...
  fetch(`https://www.snipes.com/c/shoes?sz=2000&abcz=${v4()}`, {
    'headers': {
    ...
    for (let product of products) {
      let name = product.querySelector('.b-product-tile-link.js-product-tile-link').textContent.replace('&#39;s', '').replace(' &amp;', '').trim()
      if (!PRODUCTS.includes(name)) {
        ...
        let sku = product.querySelector('.b-product-tile.js-product-tile').attributes['data-pid']
        let image = product.querySelector('.b-dynamic_image_content.b-product-tile-image').attributes.src
...
async function postRestockWebhook(url, name, price, image, sku) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTime()
    .setFooter("Snipes | v1.0", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
