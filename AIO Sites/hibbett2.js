...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.HIBBETT2);
...
const DATABASE_TABLE = 'hibbett2';
const SITENAME = 'HIBBETT2'
...
async function monitor(sku) {
    ...
    fetch(`https://www.hibbett.com/on/demandware.store/Sites-Hibbett-US-Site/default/Product-GetSetItem;.js?pid=${sku}&abcz=${v4()}`, {
        'headers': {
        },
        ...
        if (root.querySelector('.current-value')) {
            await helper.sleep(productCache.waittime);
            monitor(sku);
            return
        }
        //stats.success++;
        if (root.querySelector('.swatches.size')) {
            let title = root.querySelector('.item-name').attributes.title.trim().replace('Go to Product: ','')
            let price = root.querySelector('.price-sales').textContent.trim()
            let image = 'https:' + root.querySelector('.swatchanchor img').attributes.src.trim().split(' ').join('').replace('thumb', 'medium')
            let color = root.querySelector('.swatchanchor img').attributes.src.trim().split(' ').join('').replace('?$small$', '').split('-')[1]
...
async function postRestockWebhook(url, title, sku, price, image, sizeright, sizeleft, count) {
    var webhookMessage = new webhook.MessageBuilder()
        ...
        // .setTime()
        .setFooter("Hibbett | v2.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
...
async function cookologyhook(url, title, sku, price, image, sizeright, sizeleft, count) {
    var webhookMessage = new webhook.MessageBuilder()
        ...
        .setFooter("Hibbett | v2.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/894983430952063047/894983493984088084/Group_41.png')
...
async function dropPosthook(url, title, sku, price, image, sizeright, sizeleft, count) {
    var webhookMessage = new webhook.MessageBuilder()
        ...
        // .setTime()
        .setFooter("Hibbett | v2.0 • " + helper.getTime(true), 'https://cdn.discordapp.com/attachments/973659244723310643/983215242668539934/win.png')
...
async function postcopbox(url, title, sku, price, image, sizeright, sizeleft, count) {
    ...
    var webhookMessage = new webhook.MessageBuilder()
        ...
        // .setTime()
        .setFooter("Hibbett | v2.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/804617985498546177/989026781023580190/Frame_1.jpg')
...
async function postCarbonhook(url, title, sku, price, image, sizeright, sizeleft, count) {
    var webhookMessage = new webhook.MessageBuilder()
        ...
        // .setTime()
        .setFooter("Hibbett by Tachyon | v2.0 • " + helper.getTime(true), 'https://cdn.discordapp.com/attachments/998307425536983182/1000495532625645728/unknown.png')
...
async function Flipdhook(url, title, sku, price, image, sizeright, sizeleft, count) {
    var webhookMessage = new webhook.MessageBuilder()
        ...
        // .setTime()
        .setFooter("Hibbett by Tachyon | v2.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/927970321062055997/1011285624986271754/profilepic.jpg?width=601&height=601')
