...
async function startMonitoring() {
    ...
    console.log('[FINISHLINE] Monitoring all SKUs!')
}
async function monitor(sku) {
    ...
    await fetch(`https://www.finishline.com/store/browse/json/productSizesJson.jsp?productId=${productID}&styleId=${styleID}&colorId=${colorID}&productId=${v4()}`, {
        'headers': {
        ...
        if (inStock == true) {
            ...
            await fetch(`https://www.finishline.com/store/browse/gadgets/productLookupXML.jsp?productId=${productID}&styleId=${styleID}&colorId=${colorID}`, {
                'headers': {
                    'User-Agent': 'Mozilla/5.0 (compatible; MJ12bot/v1.4.1; http://www.majestic12.co.uk/bot.php?+)'
                },
                agent: new HTTPSProxyAgent(proxy),
                ...
                let url = `https://www.finishline.com/store/product/tachyon/${productID}?styleId=${styleID}&colorId=${colorID}`
...
async function postRestockWebhook(url, title, sku, price, image, sizeright, sizeleft, stock, productID, styleID, colorID) {
    let cyber = `[Cyber](https://cybersole.io/dashboard/tasks?quicktask=Finishline:${productID}) . `
    let phantom = `[Phantom](https://api.ghostaio.com/quicktask/send?site=Finishline&input=${productID}:${styleID}:${colorID}) . `
    let wrath = `[Wrath](http://localhost:32441/qt?input=https://www.finishline.com/store/product/~/${productID}?styleId=${styleID}%26colorId=${colorID})\n`
    let ganesh = `[Ganesh](http://www.ganeshbot.com/api/quicktask?STORE=FINISHLINE&PRODUCT=https://www.finishline.com/store/product/ganesh/${productID}?styleId=${styleID}&colorId=${colorID}&SIZE=8&MODE=MOBILE)  . `
    let kage = `[Kage](http://localhost:1007/quickTasks?site=finishline&sku=${productID})`
    var webhookMessage = new webhook.MessageBuilder()
        ...
        .setFooter("Finishline | v2.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
    ...
    if (msg.content.startsWith(discordBot.commandPrefix + 'monitorMultipleSKUs')) {
        ...
        for (let sku of skus) {
            ...
            try {
                ...
                errorSKUs.push(sku);
                console.log("*********FINISHLINE-SKU-ERROR*********");
                console.log("SKU: " + sku);
                console.log(err);
            }
        }
