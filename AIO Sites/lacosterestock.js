...
async function startMonitoring() {
  monitor()
  console.log('[Lacoste-Releases] Monitoring!')
}
...
async function monitor() {
  ...
  fetch(`https://www.lacoste.com/us/lacoste-men-shoes-low-rise-sneakers/?sortBy=New%20Arrivals%20Dropdown?abcz=${v4()}`, {
    'headers': {
    ...
    let products = root.querySelectorAll('.cell-5.offset-1.cell-m-11.l-vmargin-row-1');
    for (let product of products) {
      let name = product.querySelector('.simple-link.js-product-tile-link').textContent.replace('&#39;s', '').replace(' &amp;', '').trim()
      if (!PRODUCTS.includes(name)) {
        ...
        let url = product.querySelector('.simple-link.js-product-tile-link').attributes.href
        let price = product.querySelector('.text-grey.sales-price').textContent.replace('\n', '')
        let sku = url.split('/').slice(-1)[0].split('.')[0]
        let image = 'https:' + product.querySelector('.js-product-tile-hover-img.product-tile-image-hover.l-overlay').attributes['data-image']
...
async function postRestockWebhook(url, name, price, image, sku) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTime()
    .setFooter("Lacoste Releases | v1.0", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
