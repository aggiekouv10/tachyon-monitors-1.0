...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.EMILIOPUCCI);
...
async function startMonitoring() {
  monitor()
  console.log('[Emiliopucci-Releases]] Monitoring!')
}
...
async function monitor() {
  ...
  fetch(`https://www.emiliopucci.com/en-us/sets/woman/new-in?abcz=${v4()}`, {
    'headers': {
    ...
    let products = root.querySelectorAll('.productCard-wrapper');
    for (let product of products) {
      let name = product.querySelector('._1U90r').textContent
      if (!PRODUCTS.includes(name)) {
        ...
        // console.log(PRODUCTS)
        let url = "https://www.emiliopucci.com/" + product.querySelector('._38m7b').attributes.href
        let price = product.querySelector('.ACe0f').textContent
        let sku = url.split('-').size() - 1
...
async function postRestockWebhook(url, name, price, image, sku) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTitle(name)
    .setURL(url)
    .setAuthor('https://www.emiliopucci.com', '', 'https://www.emiliopucci.com')
