...
const discordWebhook = new webhook.Webhook('https://discord.com/api/webhooks/1014261274089107548/JR6SWMBbB722Vo8NXyDki7pdnmIOkLT9lfV443EOFvxZ9XJHugMhXRnJzjshqU9SR6Ms');
...
const DATABASE_TABLE = 'nordstrom3';
const SITENAME = 'NORDSTROM'
const CHANNEL = '1014260670369374321'
...
async function monitor(sku) {
    ...
    let cookie = await cookies[Math.floor(Math.random() * cookies.length)];
    let productCache = PRODUCTS[sku]
    if (!productCache)
        return;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 2000)
    await fetch(`https://api.nordstrom.com/offer/${sku}?apikey=kHeUEacPjrHY8SZXnLC8qGYSPXK0XJX5`, {
        'headers': {
        ...
        if (body.sellingControls[0].availability.quantity > 0) {
            ...
            try { image = body.mediaExperiences.carouselsByColor[0].orderedShots[0].url }
            catch (e) { }
            let stock = 0
            let price = ''
...
async function postElphent(sizes, sku, title, price, image) {
    const options = {
        method: 'POST',
        url: 'https://cloudapii.herokuapp.com/nord',
...
async function postRestockWebhook(url, title, sku, price, image, sizeright, sizeleft, stock) {
    var webhookMessage = new webhook.MessageBuilder()
        ...
        .setFooter("Nordstrom | v3.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
...
async function postcopbox(url, title, sku, price, image, sizeright, sizeleft, stock) {
    var webhookMessage = new webhook.MessageBuilder()
        ...
        .setFooter("Nordstrom | v3.0 • " + helper.getTime(true), 'https://media.discordapp.net/attachments/804617985498546177/989026781023580190/Frame_1.jpg')
