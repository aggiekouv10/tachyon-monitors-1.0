...
async function startMonitoring() {
  monitor()
  console.log('[Snipesusa-Releases] Monitoring!')
}
...
async function monitor() {
  ...
  const controller = new AbortController();
    ...
    fetch(`https://www.snipesusa.com/mens-shoes/?sz=2000&abcz=${v4()}`, {
    'headers': {
      'authority': 'www.snipesusa.com',
      'method': 'GET',
...
signal: controller.signal
    ...
    let products = root.querySelectorAll('.col-6.col-lg-4');
    for (let product of products) {
      let name = product.querySelector('.link.product-name').textContent.replace('&#39;s','').replace(' &amp;','').trim()
      if (!PRODUCTS.includes(name)) {
        PRODUCTS.push(name)
        if (justStarted) {
          continue;
        }
        let sku = product.querySelector('.link.product-name').attributes.href.split('-').slice(-1)[0].replace('.html','')
        let image = 'https://imageresize.24i.com/?w=300&url=' + product.querySelector('.tile-image.js-blazy').attributes['data-src'].replace('https://cdn-vzn.yottaa.net/5db9e19e4f1bbf5af87df807/','')
        let url = `https://www.snipes.com/p/tachyon-${sku}.html`
        let price = product.querySelector('.value').textContent.trim()
...
async function postRestockWebhook(url, name, price, image, sku) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTime()
    .setFooter("Snipesusa | v1.0", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
