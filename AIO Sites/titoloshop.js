...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.TITOLO);
...
async function startMonitoring() {
  monitor()
  console.log('[Titoloshop-Releases] Monitoring!')
}
...
async function monitor() {
  ...
  const controller = new AbortController();
    ...
    fetch(`https://www.titoloshop.com/eu_en/new-arrivals?abcz=${v4()}`, {
    'headers': {
...
signal: controller.signal
    ...
    let products = root.querySelectorAll('.cs-grid-layout__brick');
    for (let product of products) {
      let name = product.querySelector('.cs-product-tile__name-link.product-item-link').textContent.replace('&#39;s','').replace(' &amp;','').trim()
      if (!PRODUCTS.includes(name)) {
        ...
        let url = product.querySelector('.cs-product-tile__thumbnail-link.product-item-photo').attributes.href
        let price = product.querySelector('.price').textContent.replace('&nbsp;','').trim()
        let image = product.querySelector('.lazyload.lazyloaded').attributes.src
...
async function postRestockWebhook(url, name, price, image, sku) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTitle(name)
    .setURL(url)
    .setAuthor('https://www.titoloshop.com', '', 'https://www.titoloshop.com')
    ...
    .setTime()
    .setFooter("Titolo | v1.0", 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
