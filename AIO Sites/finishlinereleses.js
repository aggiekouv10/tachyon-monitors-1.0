...
async function startMonitoring() {
  monitor()
  console.log('[Finishline-Releases] Monitoring!')
}
...
async function monitor() {
  ...
  fetch(`https://www.finishline.com/store/men/shoes/nike/jordan/adidas/new-balance/_/N-1737dkjZhtjl46Zvnhst2Zh51uarZ1alnhbg?mnid=men_shoes_nike_jordan_adidas_newbalance&Ns=sku.daysAvailable%7C0&sort=sort%3Anewest%0A&abcz=${v4()}`, {
    'headers': {
    ...
    let products = root.querySelectorAll('.product-card');
    for (let product of products) {
      let name = product.querySelector('.product-name').textContent.replace('&#39;', "'").replace(' &amp;', '')
      if (!PRODUCTS.includes(name)) {
        ...
        // console.log(PRODUCTS)
        let url = 'https://www.finishline.com' + product.querySelector('.hover-underline').attributes.href
        let price = product.querySelector('.fullPrice').textContent.trim()
        let sku = product.attributes['data-prodid']
