...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.ZALANDOFR);
const helper = require('../helper');
let site = 'https://www.zalando.fr'
startMonitoring();
let PRODUCTS = [];
...
async function startMonitoring() {
  monitor()
  console.log('[Zalando-FR-Releases] Monitoring!')
}

async function monitor() {
  const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000)
    fetch(`${site}/chaussures-homme/?order=activation_date`, {
    'headers': {
