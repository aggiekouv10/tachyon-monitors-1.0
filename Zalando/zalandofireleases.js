...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.ZALANDOFI);
const helper = require('../helper');
let site = 'https://www.zalando.fi'
startMonitoring();
let PRODUCTS = [];
...
async function startMonitoring() {
  monitor()
  console.log('[Zalando-FI-Releases] Monitoring!')
}

async function monitor() {
  const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000)
    fetch(`${site}/miehet-kengat/?order=activation_date`, {
    'headers': {
