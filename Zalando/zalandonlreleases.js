...
const discordWebhook = new webhook.Webhook(discordBot.webhooks.ZALANDONL);
const helper = require('../helper');
let site = 'https://www.zalando.nl'
startMonitoring();
let PRODUCTS = [];
...
async function startMonitoring() {
  monitor()
  console.log('[Zalando-NL-Releases] Monitoring!')
}

async function monitor() {
  const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000)
    fetch(`${site}/schoenen/?order=activation_date`, {
    'headers': {
