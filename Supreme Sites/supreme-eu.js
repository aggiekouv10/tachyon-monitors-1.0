...
const STOCK_URLS = [
  'https://www.supremenewyork.com/mobile/products.json',
  'https://www.supremenewyork.com/shop.json',
...
async function startMonitoring() {
  ...
  // }
  monitorProductList();
  console.log("[SUPREME-EU] Started monitoring all SKUs!")
}

async function monitorProductList(sku) {
  let pdpURL = `${STOCK_URLS[helper.getRandomNumber(0, STOCK_URLS.length)]}?${v4()}`;

  let proxy = helper.getRandomSupremeEUProxy();
...
signal: controller.signal
    ...
    let totalProducts = [];
    // console.log("Fetched - " + responseTime + "ms" + " - " + pdpURL + " - " + (currentHash.includes('euro')) + " - " + proxy)
    // if (currentHash === HASH) {
...
async function postRestockWebhook(url, title, sku, sizes, price, image) {
  var webhookMessage = new webhook.MessageBuilder()
    ...
    .setTime()
    .setFooter("SUPREME-EU | v1.0 " + helper.getTime(true), 'https://media.discordapp.net/attachments/820804762459045910/821401274053820466/Copy_of_Copy_of_Copy_of_Copy_of_Untitled_5.png?width=829&height=829')
